#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Process complete product list from user"""
...
# Read from user's message - full list
products_list = """air-jordan-10,Air Jordan 10 - White brown40-47,$350.00,$39.00,https://www.ubzy.ru/product/air-jordan-10-white-brown40-47/
...
for line in products_list.strip().split('\n'):
    if not line.strip():
        continue
    
    # Simple split by comma (last part is URL)
    parts = line.rsplit(',', 4)  # Split from right, max 5 parts
    if len(parts) >= 4:
